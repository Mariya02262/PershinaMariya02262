...
c = a + (+b), поэтому программа успешно транслируется без ошибок.(py.png)


### задание 3
* токены: int, a, =, 1, ,, b, =, 2, ;, int, c, =, a, ++, +, b, ;
* значения после выполнения: a=2, b=2, c=3
Три плюса +++ разбиваются на ++ и +. Выражение работает как c = (a++) + b. (cpp.png)
* влияние пробелов: Да, изменяя пробелы, можно изменить разбиение на токены.
Например, запись int c = a + ++b ; заставит анализатор применить префиксный инкремент к переменной b. (cpp1.png)
